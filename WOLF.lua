...
ControllerAll = false
local ListSudos ={Sudo_Id,5151007856,5194316700,5151007856}  
...
Status = 'ككينـج يحـتهةة.♡"'
elseif tonumber(UserId) == 5194316700  then
Status = 'هيـرو الغشـيم•♕ '
...
msg.Name_Controller = 'ككينـج يحـتهةة.♡" '
msg.The_Controller = 1
elseif tonumber(msg.sender.user_id) == 5194316700 then
msg.Name_Controller = 'هيـرو الغشـيم•♕  '
msg.The_Controller = 1
...
local TestText = "  ❲ 𝙳𝙴𝚅𝙴𝙻𝙾𝙿𝙴𝚁𝚂 𝙺𝙸𝙽𝙶  ❳\n— — — — — — — — —\n ◍*Dev Name* :  [".. ban.first_name.."](tg://user?id="..Sudo_Id..")\n◍ *Dev Bio* : [❲ "..Bio.." ❳]"
local msg_id = msg.id/2097152/0.5 
...
if text == 'مالك السورسات' or text == 'كينج' or text == 'مالك السورس' then  
local UserId_Info = LuaTele.searchPublicChat("TR_E2S_ON_MY_MOoN")
if UserId_Info.id then
local  ban = LuaTele.getUser(UserId_Info.id)
local  bain = LuaTele.getUserFullInfo(5151007856)
if  bain.bio then
Bio =  bain.bio
else
Bio = 'لا يوجد'
...
if text == 'الغشيم' or text == 'هيرو' or text == 'احمد' then  
...
if text == 'كينج' or text == 'كينج' or text == 'المبرمج كينج' then  
local UserId_Info = LuaTele.searchPublicChat("TR_E2S_ON_MY_MOoN")
if UserId_Info.id then
local  ban = LuaTele.getUser(UserId_Info.id)
local  bain = LuaTele.getUserFullInfo(UserId_Info.id)
...
if text == '' or text == '' then  
local UserId_Info = LuaTele.searchPublicChat("SOo_/408")
if UserId_Info.id then
local  ban = LuaTele.getUser(UserId_Info.id)
local  bain = LuaTele.getUserFullInfo(UserId_Info.id)
if  bain.bio then
...
if text == 'كينج' or text == 'كينج' then  
local Get_Chat = LuaTele.getChat(msg_chat_id)
local Info_Chats = LuaTele.getSupergroupFullInfo(msg_chat_id)
local bains = LuaTele.getUser(msg.sender.user_id)
if bains.first_name then
...
if tonumber(Message_Reply.sender.user_id) == tonumber(5194316700) then
return LuaTele.sendText(msg_chat_id,msg_id,"*😹| حبيبي دا خياا كينج مش بيتف عليه*","md",true)  
end
if tonumber(Message_Reply.sender.user_id) == tonumber(5151007856) then
return LuaTele.sendText(msg_chat_id,msg_id,"*😹| حبيبي دا كينج صاحب العظمهةة مقدرش اتف عليه*","md",true)  
end
if tonumber(Message_Reply.sender.user_id) == tonumber(WOLF) then
return LuaTele.sendText(msg_chat_id,msg_id,"*👾| هذا بوت يهبل*","md",true)  
end
...
if text == "تتجوزني"  then
local Message_Reply = LuaTele.getMessage(msg.chat_id, msg.reply_to_message_id)
local ban = LuaTele.getUser(Message_Reply.sender.user_id)
local bain = LuaTele.getUser(msg.sender.user_id)
if ban.first_name then
baniusername = '*طلب :- *['..bain.first_name..'](tg://user?id='..bain.id..')*\nالزواج من  :- *['..ban.first_name..'](tg://user?id='..ban.id..')*\nالعريس المزز ابو عيون جامده دا\n*'
else
baniusername = 'لا يوجد'
end
local reply_markup = LuaTele.replyMarkup{
...
data = {
{
{text = 'موافق 🤵‍♂️', data = Message_Reply.sender.user_id..'/zog1'},{text = 'مش موافق😩', data = Message_Reply.sender.user_id..'/zog2'}, 
},
}
}
return LuaTele.sendText(msg_chat_id,msg_id,baniusername,"md",false, false, false, false, reply_markup)
...
local Name = '** [ ❲ ‹ 𝚆𝙴𝙻𝙲𝙾𝙼𝙴 𝚃𝙾 𝚂𝙾𝚄𝚁𝙲𝙴 𝙺𝙸𝙽𝙶 ›❳](t.me/SOo_KING/408)*\n\n* [ ❲ ‹𝚃𝙷𝙴𝚂𝚃𝚁𝙾𝙽𝙶𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙸𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼›❳](t.me/SOo_KING/408)*\n* [ ❲ ‹𝚁𝚄𝙽 𝚈𝙾𝚄𝚁 𝙱𝙾𝚃 𝚆𝙸𝚃𝙷 𝚄𝚂 𝙽𝙾𝚆›❳](t.me/SOo_KING/408)*\n*'
keyboard = {} 
keyboard.inline_keyboard = {
{{text = '⚶ككينـج يحـتهةة.♡"⚶',url="https://t.me/TR_E2S_ON_MY_MOoN"},{text ='⚶هيـرو الغشـيم•♕⚶',url="https://t.me/H_E_R_O_V_I_P"}},
{{text = '⚶¹'𝐵𝑂𝑇⚶',url="https://t.me/Hero9683bot"},{text ='⚶²'𝐵𝑂𝑇⚶',url="https://t.me/KkINGg701_bot"}}, 
...
{
{text = ' ❲ ‹ ⚶𝑆𝑂𝑈𝑅𝐶𝐸 ??𝐼𝑁𝐺⚶ ›❳', url = 't.me/SOo_KING/408'}, 
},
}
}
...
{
{text = '♡ ضـفني لجــروبــك يحــتهةة', url = 't.me/'..UserBot..'?startgroup=new'}, 
},
}
local msgg = msg_id/2097152/0.5
